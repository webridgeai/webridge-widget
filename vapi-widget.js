...
<div id="ss">
  ...
  function showSummary(cn) {
    fetch('https://api.jsonbin.io/v3/b/' + bid + '/latest?t=' + Date.now())
      ...
      .then(function (d) {
        ...
        // Queue fields to type in one by one
        var fields = [
          ['s-name',     cn || 'Unknown'],
          ['s-src',      'WeBridge AI Demo'],
          ['s-intent',   a.intent || '—'],
          ['s-budget',   a.budget || 'Not mentioned'],
          ['s-timeline', a.timeline || 'Not mentioned'],
          ['s-loc',      a.location || 'Not mentioned'],
          ['s-notes',    a.notes || '—'],
        ];
        ...
        function next() {
          if (idx >= fields.length) {
            // type temperature last with colour class
            typeIn(tempEl, tempTxt, function () {
              // mark saved
              setTimeout(function () {
          ...
          var f = fields[idx++];
          typeIn(gi(f[0]), f[1], next);
        }
        next();
      })
      .catch(function (e) {
        console.log('Summary fetch error:', e);
        // Still flip to summary screen even if fetch fails
  ...
  function startCall(cn) {
    ...
    s += '  document.getElementById("st").textContent="Generating summary...";';
    s += '  var c=20;';
    ...
    s += '    v.on("speech-end",function(){document.getElementById("av").style.background="linear-gradient(135deg,#1d4ed8,#7c3aed)";document.getElementById("wv").classList.remove("sp");document.getElementById("st").textContent="Listening...";});';
