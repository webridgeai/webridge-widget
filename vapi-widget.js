...
(function () {
  ...
  function startCall(cn) {
    ...
    var code = [
      ...
      '      fetch("https://api.jsonbin.io/v3/b/69b28821b7ec241ddc60e586/latest?t="+Date.now(),{headers:{"X-Master-Key":"$2a$10$aB8tvnx0wuM6EfxD.3Lm0OKcciC5VDS24yCQfSJhuIfid2Mr6tWcy"}})',
      '        .then(function(r){return r.json();})',
      '        .then(function(d){',
      '          var a=d.record||{};',
      '          var gi=function(x){return document.getElementById(x);};',
      '          var temp=(a.temperature||"warm").toLowerCase();',
      '          var tempTxt=temp==="hot"?"\uD83D\uDFE2 Hot":temp==="cold"?"\u26AA Cold":"\uD83D\uDFE1 Warm";',
      '          gi("s-name").textContent="' + cn + '";',
      '          gi("s-src").textContent="WeBridge AI Demo";',
      '          gi("s-intent").textContent=a.intent||"\u2014";',
      '          gi("s-budget").textContent=a.budget||"Not mentioned";',
      '          gi("s-timeline").textContent=a.timeline||"Not mentioned";',
      ...
      '          gi("s-notes").textContent=a.notes||"\u2014";',
      '          var te=gi("s-temp");',
      '          te.className="hs-value "+(temp==="hot"?"hot":temp==="cold"?"cold":"warm");',
      '          te.textContent=tempTxt;',
      '          gi("sc").classList.remove("on");',
      '          gi("ss").classList.add("on");',
      '          setTimeout(function(){gi("hd").className="hs-dot saved";gi("htxt").textContent="Saved";gi("hsv").style.display="flex";},400);',
      '        })',
      '        .catch(function(e){console.log("fetch error",e);document.getElementById("sc").classList.remove("on");document.getElementById("ss").classList.add("on");});',
      '    }',
      '  },1000);',
      '}',
